...
class IW_SSIM():
    ...
    def info_content_weight_map(self, imgopr, imgdpr):
        ...
        for scale in range(1, self.Nsc):
            ...
            if self.use_double:
                L = torch.diag(eig_values * (eig_values > 0).double()) * torch.sum(eig_values) / ((torch.sum(eig_values * (eig_values > 0).double())) + (torch.sum(eig_values * (eig_values > 0).double())==0)) 
            else:
                L = torch.diag(eig_values * (eig_values > 0).float()) * torch.sum(eig_values) / ((torch.sum(eig_values * (eig_values > 0).float())) + (torch.sum(eig_values * (eig_values > 0).float()) == 0))
            ...
            for j in range(len(eig_values)):
                infow = infow + torch.log2(1 + ((vv + (1 + g * g) * self.sigma_nsq) * ss * eig_values[j] + self.sigma_nsq * vv) / (self.sigma_nsq * self.sigma_nsq))
            infow[infow < tol] = 0
            iw_map[scale] = infow
