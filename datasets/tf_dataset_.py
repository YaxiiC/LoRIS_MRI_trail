...
def denormalize(images):
    # Denormalize the images to the original range [0, 255]
...
class CustomDataGen(tf.keras.utils.Sequence):
    ...
    def __init__(self,
        ...
        self.phase = phase
        # load dataset
        self.x, self.y, self.coord, self.mask = self.load_dataset_folder()
    ...
    def load_dataset_folder(self):
        phase = self.phase
        x, y, boxes, mask = [], [], [], []
        img_dir = os.path.join('../anomaly_0', self.class_name, phase)
        box_dir = os.path.join('../anomaly_0', self.class_name, 'boxes')
        gt_dir = os.path.join('../anomaly_0', self.class_name, 'ground_truth')
    ...
    def __getitem__(self, idx):
        ...
        for i in indexes:
            ...
            # Ensure consistent number of channels (e.g., 1-channel mask)
            if msk.shape[0] != 1:
                msk = msk[:1, :, :]  # Keep only the first channel

            print(msk.shape)  # Debugging shape
